...
{
    /// <summary>
    /// Main window: CRUD for game hierarchy objects, JSON/XML persistence, plugin-driven types.
    /// Lab5 extends Lab4 with the XML Plugin pipeline and the Plugins menu.
    /// </summary>
    public partial class Form1 : Form
    {
        ...
        /// <summary>
        /// Serializes the collection to games.xml via the JSON+XML pipeline.
        /// Enabled XML plugins transform the content before it is written to disk.
        /// </summary>
        private void btnSave_Click(object sender, EventArgs e)
        {
            JsonSerializerService.Save("games.xml", items);
            MessageBox.Show("Saved to games.xml" +
                (XmlPluginManager.GetEnabledPlugins().Count > 0
                    ? "\n(XML plugins applied before save)"
                    : ""));
        }

        /// <summary>
        /// Deserializes items from games.xml.
        /// Enabled XML plugins transform the content after it is read from disk.
        /// Plugin assemblies must already be loaded.
        /// </summary>
        private void btnLoad_Click(object sender, EventArgs e)
        {
            items = JsonSerializerService.Load("games.xml");
            UpdateList();
            MessageBox.Show("Loaded from games.xml" +
                (XmlPluginManager.GetEnabledPlugins().Count > 0
                    ? "\n(XML plugins applied after load)"
                    : ""));
        }
        ...
        /// <summary>Opens the XML plugin manager dialog.</summary>
        private void menuPluginsManage_Click(object sender, EventArgs e)
        {
            using (var dlg = new FormPluginManager())
            {
                dlg.ShowDialog(this);
            }
        }

        /// <summary>Opens an OpenFileDialog to load a game-type plugin DLL manually.</summary>
        private void menuPluginsLoadDll_Click(object sender, EventArgs e)
        {
            using (var dlg = new OpenFileDialog())
            {
                dlg.Title = "Load Game Plugin DLL";
                ...
                if (dlg.ShowDialog() == DialogResult.OK)
                {
                    PluginLoader.ReloadAll();
                    RefreshTypeComboBox();
                    MessageBox.Show("Plugin DLL loaded from:\n" + dlg.FileName);
                }
            }
        }

        /// <summary>Opens an OpenFileDialog to load an XML-processing plugin DLL manually.</summary>
        private void menuPluginsLoadXmlDll_Click(object sender, EventArgs e)
        {
            int count = XmlPluginManager.LoadFromDialog();
            if (count > 0)
                MessageBox.Show(string.Format("Loaded {0} XML plugin(s).", count));
        }
